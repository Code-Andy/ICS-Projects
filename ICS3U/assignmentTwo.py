...
animalList = ["monkey", "lion", "elephant", "giraffe",
              "penguin", "polarbear", "panda", "koala", "gorilla", "hippo"]
...
def addAnimal(name, budget, amount):
    ...
    '''
    logging.info("addAnimal() begins with " + name + " name " +
                 str(budget) + " budget " + str(amount) + " amount")
    ...
    if not isinstance(lowerCase, str) or not isinstance(budget, int) or not isinstance(amount, int):
        logging.error(
            "addAnimal() | values are not the correct type for function to continue")
        raise TypeError(
            "values are not the correct type for function to continue")
    # checks parameter value
    if lowerCase not in animalList:
        logging.error("addAnimal() | " + str(lowerCase) +
                      "is not a animal in the list given")
    ...
    if amount < 0:
        logging.error("addAnimal() | " +
                      "amount of animals cannot be negative")
...
while(True):
    ...
    for x in range(0, len(animalList)):
        print("Name:", animalList[x], "|| Cost:",
              animalCost[x], "|| Revenue:", animalRevenue[x])
    # tries each line and moves on with success
    try:
        # requests zookeeper input for what and how many animals to buy
        logging.debug(
            "Asks zookeeper to select a animal and the amount they want to buy")
        ...
        logging.info("Function started with " + str(selectedAnimal) +
                     " budget of " + str(totalBudget) + " amount of " + str(animalAmount))
    ...
    # different errors for the 2 inputs(selectedAnimal, animalAmount)
        ...
        print("Added", animalAmount, selectedAnimal,
              "to the zoo, budget is now", totalBudget)
        print("--------------------------------------------------------------------\n")
        # see if budget is high enough to buy another animal
        try:
            if(totalBudget >= min(animalCost)):
                logging.debug(
                    "Asks zookeeper if he wants to acquire more animals if he can")
                userVal = input(
                    "Do you want to acquire more animals? Yes | No ")
                ...
                if(userVal.lower() != "yes" and userVal.lower() != "no"):
                    logging.info(
                        "ValueError, value is not 'Yes' or 'No', moving on as no")
        ...
        # additional errors for userVal input
            ...
            logging.debug(
                "No errors from zookeeper selection, continuing to revenue statistics")
            ...
            if(totalBudget < min(animalCost) or userVal.lower() == "no"):
                ...
                for x in range(0, len(animalList)):
                    ...
                    print(animalList[x], "created", offSpring(
                        animalCount[x]), "offsprings")

                print("\nYou currently have:\n")
                for x in range(0, len(animalList)):
                    print(animalCount[x], animalList[x], "(s)", "generating",
                          animalRevenue[x] * animalCount[x], "credits per month")
                ...
                print("Your new budget for next month is",
                      totalBudget, "credits\n")
            else:
                # zookeeper requests to add another animal within the same month, loop the program again with a different starting line
                logging.debug(
                    "zookeeper requested to buy another animal, loop is repeating once more")
                print("\nYour budget currently is", totalBudget,
                      "Please select another animal to add\n")
